...
namespace depth_image_filters
{
    ...
    cv::Rect CropFilter::computeCropRect(int img_width, int img_height) const
    {
        ...
        return cv::Rect(
            left,
            top,
            img_width - left - right,
            img_height - top - bottom
        );
    }

    bool CropFilter::validateRect(const cv::Rect& rect,
                                  int img_width, int img_height) const
    {
        if (rect.width <= 0 || rect.height <= 0)
        {
            RCLCPP_ERROR(logger_,
                         "Margin too large — resulting size: %dx%d",
        ...
        {
            RCLCPP_ERROR(logger_,
                         "Crop rect out of bounds: (%d,%d) %dx%d vs image %dx%d",
