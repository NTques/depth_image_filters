...
bool RangeFilter::apply(cv::Mat &image, const std::string &encoding,
  ...
  if (!is_float && !is_uint16) {
    RCLCPP_ERROR(logger_, "Unsupported encoding: %s (expected 32FC1 or 16UC1)",
                 encoding.c_str());
    return false;
  }
  ...
  if (is_float) {
    ...
    constexpr double kUint16Max =
        static_cast<double>(std::numeric_limits<uint16_t>::max());
    const uint16_t min_val = static_cast<uint16_t>(
        std::clamp(p.min_distance * 1000.0, 0.0, kUint16Max));
    const uint16_t max_val = static_cast<uint16_t>(
        std::clamp(p.max_distance * 1000.0, 0.0, kUint16Max));
